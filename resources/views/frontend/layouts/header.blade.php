        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-6 col-md-12 col-12">
                    ...
                    <div class="top-left">
                        <ul class="list-main">
                            <!-- Xóa đoạn lấy settings và foreach settings vì không còn bảng settings -->
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-2 col-md-2 col-12">
                    <!-- Logo -->
                    <div class="logo">
                        <a href="http://127.0.0.1:8000/"><img src="{{asset('/storage/photos/33/Logo/văn phòng phẩm.png')}}" alt="#"></a>
                    </div>
                    <!--/ End Logo -->
                    <!-- Search Form -->
                    ...
                    <div class="search-bar-top">
                        <div class="search-bar">
                            <select>
                                ...
                                @foreach(\App\Helpers::getAllCategory() as $cat)
                    ...
                    <div class="right-bar">
                        ...
                        <div class="sinlge-bar shopping">
                           ...
                           @if(session('wishlist'))
                                ...
                                @endforeach
                           @endif
                            <a href="{{route('wishlist')}}" class="single-icon"><i class="fa fa-heart-o"></i> <span class="total-count">{{\App\Helpers::wishlistCount()}}</span></a>
                            <!-- Shopping Item -->
                            @auth
                                <div class="shopping-item">
                                    <div class="dropdown-cart-header">
                                        <span>{{count(\App\Helpers::getAllProductFromWishlist())}} Sản phẩm</span>
                                        <a href="{{route('wishlist')}}">Xem danh sách yêu thích</a>
                                    </div>
                                    <ul class="shopping-list">
                                        {{-- {{\App\Helpers::getAllProductFromCart()}} --}}
                                            @foreach(\App\Helpers::getAllProductFromWishlist() as $data)
                                                    @php
                                            ...
                                            <li>
                                                ...
                                                <p class="quantity">
                                                    {{ $data->quantity }} x -
                                                    <span class="amount">{{ number_format($data->price, 0) }} đ</span>
                                                </p>
                                            </li>
                                        @endforeach
                                    </ul>
                                    <div class="bottom">
                                        <div class="total">
                                            ...
                                            <span class="total-amount">{{number_format(\App\Helpers::totalWishlistPrice(),0)}} đ</span>
                                        </div>
                        ...
                        <div class="sinlge-bar shopping">
                            <a href="{{route('cart')}}" class="single-icon"><i class="ti-bag"></i> <span class="total-count">{{\App\Helpers::cartCount()}}</span></a>
                            <!-- Shopping Item -->
                            @auth
                                <div class="shopping-item">
                                    <div class="dropdown-cart-header">
                                        <span>{{count(\App\Helpers::getAllProductFromCart())}} Sản Phẩm</span>
                                        <a href="{{route('cart')}}">Xem Giỏ Hàng</a>
                                    </div>
                                    <ul class="shopping-list">
                                        {{-- {{\App\Helpers::getAllProductFromCart()}} --}}
                                            @foreach(\App\Helpers::getAllProductFromCart() as $data)
                                            @php
                                                $photo = explode(',', $data->product['photo']);
                                            @endphp
                                            <li>
                                                <a href="{{ route('cart-delete', $data->id) }}" class="remove" title="Xóa sản phẩm"><i class="fa fa-remove"></i></a>
                                                <a class="cart-img" href="#"><img src="{{ $photo[0] }}" alt="{{ $photo[0] }}"></a>
                                                <h4><a href="{{ route('product-detail', $data->product['slug']) }}" target="_blank">{{ $data->product['title'] }}</a></h4>
                                                <p class="quantity">
                                                    {{ $data->quantity }} x - <span class="amount">{{ number_format($data->price, 0) }}đ</span>
                                                </p>
                                            </li>
                                    ...
                                    <div class="bottom">
                                        <div class="total">
                                            <span>Tổng số tiền</span>
                                            <span class="total-amount">{{number_format(\App\Helpers::totalCartPrice(),0)}} đ</span>
                                        </div>
